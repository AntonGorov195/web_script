...
async function ExecWasm(src, extraImports) {
    ...
    { // Init
        ...
        wasm.view = () => new DataView(wasm.mem.buffer);
        wasm.reader = {
            Bytes: (ptr, len) => {
        ...
        wasm.writer = {
            ...
            Int: function (val, ptr) { wasm.intSize === 4 ? wasm.view().setInt32(ptr, val) : wasm.view().setInt64(ptr, val) },
            UInt: function (val, ptr) { wasm.intSize === 4 ? wasm.view().setUInt32(ptr, val) : wasm.view().setUInt64(ptr, val) },
        }
        wasm.imports = {
            env: {
                memory: wasm.mem,
            ...
            web_odin: {
                ...
                _alloc_combo_all_func: function(func_id) {
                    const func = wasm.jsmem.vals[func_id];
                    const combo = (...args) => {
                        const args_as_id = []; 
                        for(const arg in args) {
                            const arg_id = wasm.jsmem.alloc(arg); 
                            args_as_id.push(arg_id);
                        }
                        func(...args_as_id)
                        for(const arg_id in args_as_id) {
                            wasm.jsmem.free(arg_id); 
                        }
                    }
                    return wasm.jsmem.alloc(combo);
                }
            },
            ...extraImports,
        };
